<template>
    <div v-if="!vod?.segments || vod.segments.length == 0" class="notification is-error">
    ...
    <div class="field">
        ...
        <div class="control">
            ...
            <p v-if="exporter == 'file' || exporter == 'sftp' || exporter == 'ftp' || exporter == 'rclone'" class="template-preview">
                {{ templatePreview(exportVodSettings.title_template || "") }}.mp4
            </p>
            <p v-else-if="exporter == 'youtube'" class="template-preview">
    ...
    <div v-if="exporter == 'rclone'" class="field">
        ...
        <div class="control has-addon">
            ...
            <button class="button is-confirm" :title="t('vod.export.get-remotes')" @click="getRemotes">
                <span class="icon">
                    <fa icon="sync" :spin="LoadingRemotes" />
                </span>
            </button>
        </div>
    </div>
    ...
    <div v-if="exporter == 'youtube'" class="field">
        ...
        <div class="control has-addon">
            <div class="select">
                <select v-model="exportVodSettings.playlist_id" :disabled="LoadingPlaylists">
                    <option value="">({{ t("messages.none") }})</option>
                    <option v-for="(p, i) in YouTubePlaylists" :key="i" :value="p.id">
                        {{ p.snippet.title }}
                    </option>
                </select>
            </div>
            <button class="button is-confirm" :title="t('vod.export.get-playlists')" @click="getYouTubePlaylists">
                <span class="icon">
                    <fa icon="sync" :spin="LoadingPlaylists" />
                </span>
            </button>
            <button class="button is-confirm" :title="t('vod.export.create-playlist')" @click="createYouTubePlaylist">
                <span class="icon">
    ...
    <div class="field">
        <div class="control">
            <d-button color="success" icon="upload" :loading="loading" @click="doExportVod">
                {{ t("buttons.export") }}
            </d-button>
        </div>
    </div>
...
function doExportVod() {
    ...
    loading.value = true;
    axios
        .post<ApiResponse>(`/api/v0/exporter?mode=vod&exporter=${exporter.value}`, exportVodSettings.value)
        .then((response) => {
...
function getYouTubePlaylists() {
    ...
    axios
        .get<ApiResponse>("/api/v0/youtube/playlists")
        .then((response) => {
            const json = response.data;
            if (json.message) alert(json.message);
            console.log(json);
...
function createYouTubePlaylist() {
    ...
    axios
        .post<ApiResponse>("/api/v0/youtube/playlists", {
            title: title,
...
function getRemotes() {
    ...
    axios
        .get<ApiResponse>("/api/v0/exporter/rclone/remotes")
        .then((response) => {
            const json = response.data;
            if (json.message) alert(json.message);
            console.log(json);
