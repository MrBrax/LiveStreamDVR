<template>
    <!-- bookmark list -->
    <div v-if="vod.provider == 'twitch'" class="video-block video-bookmarks" aria-label="Bookmarks">
        <div class="video-block-header collapsible" aria-role="button" @click="isCollapsed = !isCollapsed">
            <h4>
                ...
                {{ t("vod.bookmarks") }} <span class="amount" :data-amount="vod.bookmarks.length">({{ vod.bookmarks.length }})</span>
            </h4>
        </div>
        <transition name="blinds">
            <div v-if="!isCollapsed" class="video-block-content">
                <ul v-if="vod.bookmarks && vod.bookmarks.length > 0" class="list-segments">
                    <li v-for="(bookmark, i) in vod.bookmarks" :key="i">
                        <router-link :to="playerLink(bookmark)"> {{ humanDuration(bookmark.offset || 0) }} - {{ bookmark.name }} </router-link>
                        &nbsp;
                        <button class="icon-button" @click="doDeleteBookmark(i)">
                ...
                <details class="details">
                    ...
                    <div class="field">
                        <label class="label" :for="`name.${vod.uuid}`">Name</label>
                        <input :id="`name.${vod.uuid}`" v-model="newBookmark.name" class="input" type="text" />
                    </div>
                    <div v-if="vod.is_finalized" class="field">
                        <label class="label" :for="`offset.${vod.uuid}`">Offset</label>
                        <input :id="`offset.${vod.uuid}`" v-model="newBookmark.offset" class="input" type="number" />
                    </div>
                    <d-button size="small" color="success" icon="plus" @click="doMakeBookmark">
                        {{ t("buttons.create") }}
                    </d-button>
                </details>
            </div>
...
function doMakeBookmark() {
    if (!props.vod) return;
    axios
        .post<ApiResponse>(`/api/v0/vod/${props.vod.uuid}/bookmark`, newBookmark.value)
        .then((response) => {
...
function doDeleteBookmark(i: number) {
    if (!props.vod) return;
    axios
        .delete<ApiResponse>(`/api/v0/vod/${props.vod.uuid}/bookmark?index=${i}`)
        .then((response) => {
            const json = response.data;
            if (json.message) alert(json.message);
            console.log(json);
