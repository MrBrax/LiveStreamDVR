<template>
    <div class="field">
        <label class="label" for="absolute-season">{{ t("vod.edit.absolute-season") }}</label>
        <div class="control">
            <input id="absolute-season" v-model.number="editVodSettings.absolute_season" class="input" type="number" />
        </div>
    </div>

    <div class="field">
        <label class="label" for="stream-number">{{ t("vod.edit.stream-number") }}</label>
        <div class="control">
            <input id="stream-number" v-model.number="editVodSettings.stream_number" class="input" type="number" />
        </div>
    </div>

    <div class="field">
        <label class="label" for="stream-number">{{ t("vod.edit.absolute-stream-number") }}</label>
        <div class="control">
            <input id="stream-number" v-model.number="editVodSettings.absolute_stream_number" class="input" type="number" />
        </div>
    </div>

    <div class="field">
        <label class="label" for="comment">{{ t("vod.edit.comment") }}</label>
        <div class="control">
            <textarea id="comment" v-model="editVodSettings.comment" class="input textarea" />
        </div>
    </div>

    <div class="field">
        <label class="label" for="segments">{{ t("vod.edit.segments") }}</label>
        <div class="control">
            <textarea id="segments" v-model="editVodSettings.segments" class="input textarea" />
        </div>
    </div>

    <div class="field">
        <label class="label">{{ t("vod.edit.chapters") }}</label>
        <div class="control">
            <label class="checkbox">
                <input v-model="editVodSettings.editChapters" type="checkbox" />
        ...
        <div v-if="editVodSettings.editChapters">
            ...
            <table class="table is-fullwidth">
                <tr v-for="(chapter, i) in sortedChapters" :key="chapter.originalIndex">
                    <td>
                        {{ chapter.originalIndex }}
                    </td>
                    <td>
                        <input v-model.lazy="chapter.offset" type="number" class="input is-small" />
                    </td>
                    <!-- offset -->
                    <td>
                        <input :value="humanReadableChapterOffset(i)" readonly disabled class="input is-small" />
                    </td>
                    <!-- date -->
                    <td>
                        <input :value="chapterDate(i)?.toISOString()" readonly disabled class="input is-small" />
                    </td>
                    <!-- title -->
                    <td>
                        <input v-model="chapter.title" type="text" class="input is-small" />
                    </td>
                    <!-- game id -->
                    <td>
                        <div class="select is-small">
                            <select v-if="vod.provider == 'twitch'" v-model="chapter.game_id">
                                <option value="">None</option>
                                <option v-for="game in sortedGamesData" :key="game.id" :value="game.id">
                                    {{ game.name }}
                                </option>
                            </select>
                        </div>
                    ...
                    <!-- viewer count -->
                    <td>
                        <input :value="chapter.viewer_count" readonly disabled class="input is-small" />
                    </td>
                    <!-- delete -->
                    <td>
                        <button class="button is-small is-danger" @click="deleteChapter(i)">
                            <span class="icon">
                ...
                <tr>
                    <td />
                    <td>
                        <input v-model="newChapter.offset" type="number" class="input is-small" />
                    </td>
                    <td>
                        <input :value="humanDuration(newChapter.offset)" readonly disabled class="input is-small" />
                    </td>
                    <td>
                        <input
                    ...
                    <!-- title -->
                    <td>
                        <input v-model="newChapter.title" type="text" class="input is-small" />
                    </td>
                    <!-- game id -->
                    <td>
                        <div class="select is-small">
                            <select v-if="vod.provider == 'twitch'" v-model="newChapter.game_id">
                                <option value="">None</option>
                                <option v-for="game in sortedGamesData" :key="game.id" :value="game.id">
                                    {{ game.name }}
                                </option>
                    ...
                    <td>
                        <input :value="newChapter.viewer_count" readonly disabled class="input is-small" />
                    </td>
                    <td>
                        <button class="button is-small is-confirm" @click="addChapter">
                            <span class="icon">
            ...
            <table class="chaptervis">
                <tr>
                    <td v-if="sortedChapters.length > 0 && sortedChapters[0].offset > 0" :style="{ left: `${firstChapterLeft}%` }" />
                    <td v-for="(chapter, i) in sortedChapters" :key="i" :style="{ width: `${chapterWidth(i)}%` }">
            ...
            -->
            <d-button size="small" color="danger" icon="undo" @click="resetChapters">
                {{ t("vod.edit.reset-chapters") }}
            </d-button>
            <p>Total duration: {{ vod.duration }}</p>
        </div>
    </div>
    ...
    <div class="field">
        <div class="control">
            <label class="checkbox">
                <input v-model="editVodSettings.prevent_deletion" type="checkbox" />
    ...
    <div class="field">
        <div class="control">
            <label class="checkbox">
                <input v-model="editVodSettings.cloud_storage" type="checkbox" />
    ...
    <div class="field">
        <d-button color="success" icon="save" @click="doEditVod">
            {{ t("buttons.save") }}
        </d-button>
    </div>
</template>
...
function doEditVod() {
    ...
    sortChapters();
    axios
        .post<ApiResponse>(`/api/v0/vod/${props.vod.uuid}`, editVodSettings.value)
        .then((response) => {
...
onMounted(() => {
    ...
    axios.get<ApiGamesResponse>("/api/v0/games").then((response) => {
        const json = response.data;
        if (json.message) alert(json.message);
