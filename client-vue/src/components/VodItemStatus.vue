<template>
    <div v-if="(vod.failed && !vod.is_finalized && !vod.is_capturing) || vod.hasError()" class="video-block video-error">
        <!-- capture failed-->
        <div class="video-block-content">
            <strong>
                <span class="icon"><font-awesome-icon icon="exclamation-triangle" /></span> {{ t("vod.failed") }} </strong
            >&nbsp;
            <div class="buttons">
                <!-- Delete -->
                <d-button class="is-danger" color="danger" size="small" icon="trash" :disabled="vod.prevent_deletion" @click="emit('delete')">
                    {{ t("buttons.delete") }}
                </d-button>

                <!-- Fix issues -->
                <d-button color="success" size="small" icon="wrench" @click="emit('fixIssues')">
    ...
    <div v-else-if="!vod.is_finalized" class="video-block video-status">
        ...
        <div class="video-block-content">
            ...
            <template v-else-if="vod && vod.is_capturing">
                <em class="text-overflow">
                    ...
                    >)
                    <button class="icon-button" title="Refresh" @click="vod && store.fetchAndUpdateVod(vod.uuid)">
    ...
    <div v-if="vod.is_capturing && durationIsLongerThan24Hours(vod)" class="video-block video-error">
    ...
    <div v-if="!vod.chapters" class="video-block video-error">
        <!-- no chapters error -->
        <div class="video-block-content">No chapter data!?</div>
    </div>
</template>
