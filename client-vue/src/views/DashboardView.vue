<template>
    <div class="container vertical">
        <article v-if="store.errors && store.errors.length > 0 && store.authElement" class="errors" aria-label="Errors">
        ...
        <section v-if="store.cfg('motd')" class="section">
        ...
        <section class="section" data-section="vods">
            ...
            <div v-if="store.streamerListLoaded && store.streamerList.length > 0" class="section-content">
                <template v-if="!store.clientCfg('singlePage')">
                    <streamer v-for="streamer in sortedStreamers" :key="streamer.uuid" :streamer="streamer" />
                </template>
                <template v-else>
                    <streamer v-if="singleStreamer" :streamer="singleStreamer" @refresh="store.fetchAndUpdateStreamerList" />
                </template>
                <hr />
            ...
            <div v-else-if="!store.streamerListLoaded && store.authElement" class="section-content">
                <LoadingBox />
            </div>
            <div v-else-if="!store.authElement" class="section-content">
        ...
        <section class="section">
            <div class="section-title is-expandable" tabindex="0" @click="logToggle" @keyup.prevent.enter="logToggle" @keyup.prevent.space="logToggle">
                <h1>{{ t("dashboard.logs") }}</h1>
            </div>
            <div v-if="logVisible && store.authElement" class="section-content">
