 ...
 * @returns
 */
export function execSimple(bin: string, args: string[], what: string): Promise<ExecReturn> {
 ...
 * @returns
 */
export function execAdvanced(bin: string, args: string[], jobName: string, progressFunction?: (log: string) => number | undefined): Promise<ExecReturn> {
    return new Promise((resolve, reject) => {
...
export function startJob(jobName: string, bin: string, args: string[], env: Record<string, string> = {}): Job | false {
...
export function GetRunningProcessByPid(pid: number): RunningProcess | undefined {
