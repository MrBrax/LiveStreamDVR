...
export class TwitchVOD extends BaseVOD {
    ...
    public json?: TwitchVODJSON;

    public chapters_raw: Array<TwitchVODChapterJSON> = [];
    public chapters: Array<TwitchVODChapter> = [];

    /** @deprecated Use external_vod_id instead */
    public twitch_vod_id?: string;
    /** @deprecated Use external_vod_duration instead */
    public twitch_vod_duration?: number;
    /** @deprecated Use external_vod_title instead */
    public twitch_vod_title?: string;
    /** @deprecated Use external_vod_date instead */
    public twitch_vod_date?: string;
    public twitch_vod_muted?: MuteStatus;
    // twitch_vod_status?: ExistStatus;
    public twitch_vod_neversaved?: boolean;
    /** @deprecated Use external_vod_exists instead */
    public twitch_vod_exists?: boolean;
    public twitch_vod_attempted?: boolean;
    ...
    public static async downloadVideo(
        ...
        if (
            ...
            !fs.existsSync(convertedFilename)
        ) {
            const videoUrl = `https://www.twitch.tv/videos/${video_id}`;

            const streamlinkBin = Helper.path_streamlink();
            ...
            cmd.push("--ffmpeg-ffmpeg", ffmpegBin);
        ...
        if (!fs.existsSync(convertedFilename)) {
            ...
            if (Config.getInstance().cfg("create_video_chapters")) {
                ...
                fs.writeFileSync(chaptersFile, meta.getString());
            }
            ...
            if (ret.success) {
                log(
                    LOGLEVEL.INFO,
                    "tw.vod.downloadVideo",
                    `Successfully remuxed ${basename}, removing ${captureFilename}`
                );
                fs.unlinkSync(captureFilename);
            } else {
                log(
                    LOGLEVEL.INFO,
                    "tw.vod.downloadVideo",
        ...
        const successful =
            fs.existsSync(convertedFilename) &&
            fs.statSync(convertedFilename).size > 0;
     ...
     * @returns
     */
    public static async getVideo(video_id: string): Promise<false | Video> {
     ...
     * @returns
     */
    public static async getVideos(ids: string[]): Promise<false | Video[]> {
     ...
     * @returns
     */
    public static async getVideosRecord(
        ids: string[]
    ...
    public static async getLatestVideos(
        channel_id: string
    ): Promise<false | Video[]> {
    ...
    public static async getVideosProxy(
        channel_id: string
    ...
    public static async getVideoProxy(
        video_id: string
    ): Promise<false | ProxyVideo> {
        if (!video_id) throw new Error("No video id");
    ...
    public static async getGqlVideoChapters(
        video_id: string
    ...
    public static async getGqlVideoInfo(video_id: string) {
    ...
    public static downloadChatTCD(
        ...
        return new Promise((resolve, reject) => {
            ...
            if (fs.existsSync(tempFilepath)) {
                fs.renameSync(tempFilepath, output);
                log(
                    LOGLEVEL.INFO,
                    "vod.downloadChatTCD",
    ...
    */
    ...
    public async parseChapters(
        ...
        for (const chapter of raw_chapters) {
            ...
            const newChapter = await TwitchVODChapter.fromJSON(chapter);
            newChapter.vod_uuid = this.uuid;
    ...
    public async splitSegmentVideoByChapters(): Promise<boolean> {
        ...
        const videoInput = path.join(this.directory, this.segments_raw[0]); // TODO: burned video etc
        ...
        for (const chapter of this.chapters) {
            ...
            chapterFilename = sanitize(chapterFilename);

            if (!validateFilename(chapterFilename)) {
                throw new Error(
                    `Invalid filename for ${this.basename}: ${chapterFilename}`
                );
            }
            ...
            args.push(
                ...
                this.realpath(videoInput)
            );
            args.push("-ss", formatSubtitleDuration(chapterStart));
            ...
            try {
                job = await execAdvanced(
                    bin,
                    args,
                    `chapter_split_${this.basename}_${chapterIndex}`
                );
            } catch (error) {
            ...
            if (
                ...
                fs.statSync(chapterPath).size == 0
            ) {
                log(
                    LOGLEVEL.ERROR,
                    "vod.splitSegmentByChapters",
    ...
    public async matchProviderVod(force = false): Promise<boolean | undefined> {
        ...
        for (const video of channelVideos) {
            ...
            const startOffset = Math.abs(
                this.started_at.getTime() - videoTime.getTime()
            );
        ...
        throw new Error(`No matching VOD from ${channelVideos.length} videos`);
    }
    ...
    public async saveVTTChapters(): Promise<boolean> {
        ...
        this.chapters.forEach((chapter, i) => {
            ...
            const txtStart = formatSubtitleDuration(start);
            const txtEnd = formatSubtitleDuration(end);

            data += `Chapter ${i + 1}\n`;
            data += `${txtStart} --> ${txtEnd}\n`;
    ...
    public async checkValidVod(save = false): Promise<boolean | null> {
        const currentStatus = this.external_vod_exists;
        ...
        if (video) {
            ...
            this.external_vod_exists = true;
            if (save && currentStatus !== this.external_vod_exists) {
