...
export class TwitchAutomator extends BaseAutomator {
    ...
    public async handle(data: EventSubResponse, metadata: AutomatorMetadata): Promise<boolean> {
        log(LOGLEVEL.DEBUG, "automator.handle", "Handle called, proceed to parsing.");

        if (!metadata.message_id) {
            log(LOGLEVEL.ERROR, "automator.handle", "No twitch message id supplied to handle");
            return false;
        }
        ...
        if (messageRetry !== undefined && messageRetry > 0) {
            log(LOGLEVEL.WARNING, "automator.handle", `Message ${messageId} is a retry (${messageRetry})`);
            if (Config.getInstance().cfg("capture.retry_on_error", true)) {
                log(LOGLEVEL.INFO, "automator.handle", `Retrying message ${messageId}`);
            } else {
                log(LOGLEVEL.WARNING, "automator.handle", `Not retrying message ${messageId}`);
                return false;
            }
        }
        ...
        try {
            ...
            log(LOGLEVEL.WARNING, "automator.handle", `Failed to set eventsub message ${messageId} KeyValues: ${(error as Error).message}`);
        }
        ...
        if (subscription_type === "channel.update") {
            ...
            if (!TwitchChannel.getChannelByLogin(this.broadcaster_user_login)) {
                log(LOGLEVEL.ERROR, "automator.handle", `Handle (update) triggered with sub id ${subscription_id}, but username '${this.broadcaster_user_login}' is not in config.`);
            ...
            log(LOGLEVEL.INFO, "automator.handle", `Automator channel.update event for ${this.broadcaster_user_login}`);

            return await this.updateGame();
        } else if (subscription_type == "stream.online") {

            if (!("id" in event)) {
                log(LOGLEVEL.ERROR, "automator.handle", `No stream id supplied in event for channel '${this.broadcaster_user_login}', aborting.`, event);
                return false;
            }
            ...
            log(LOGLEVEL.INFO, "automator.handle", `Automator stream.online event for ${this.broadcaster_user_login} (retry ${messageRetry})`);

            // const channel_obj = TwitchChannel.getChannelByLogin(this.broadcaster_user_login);

            // check if channel is in config, hmm
            if (!this.channel) {
                log(LOGLEVEL.ERROR, "automator.handle", `Handle (online) triggered with sub id ${subscription_id}, but username '${this.broadcaster_user_login}' is not in config.`);
            ...
            if (this.channel.is_live) {
                log(LOGLEVEL.INFO, "automator.handle", `${this.broadcaster_user_login} is already live, yet another stream online event received.`);
            }
            ...
            log(LOGLEVEL.INFO, "automator.handle", `${this.broadcaster_user_login} stream has ID ${event.id}, started ${event.started_at}`);
            ...
            if (TwitchVOD.hasVod(basename)) {
                log(LOGLEVEL.INFO, "automator.handle", `Channel ${this.broadcaster_user_login} online, but vod ${basename} already exists, skipping`);
            ...
            if (capture_vod) {
                log(LOGLEVEL.INFO, "automator.handle", `Channel ${this.broadcaster_user_login} online, but vod ${event.id} already exists (${capture_vod.basename}), skipping`);
                this.fallbackCapture().then(() => {
                    log(LOGLEVEL.INFO, "automator.download", `Fallback capture finished for ${this.getLogin()}`);
                }).catch(error => {
                    log(LOGLEVEL.ERROR, "automator.download", `Fallback capture failed for ${this.getLogin()}: ${(error as Error).message}`);
            ...
            if (this.channel.no_capture) {
                log(LOGLEVEL.INFO, "automator.handle", `Skip capture for ${this.broadcaster_user_login} because no-capture is set`);
            ...
            try {
                ...
                log(LOGLEVEL.ERROR, "automator.handle", `Download of stream '${this.broadcaster_user_login}' failed: ${(error as Error).message}`);
            }
            ...
            log(LOGLEVEL.ERROR, "automator.handle", `No supported subscription type (${subscription_type}).`);
            return false;
    ...
    public async updateGame(from_cache = false, no_run_check = false) {
        ...
        if (this.channel?.is_capturing) {
            ...
            if (!capture_id) {
                log(LOGLEVEL.FATAL, "automator.updateGame", `No capture ID for channel ${this.getLogin()} stream, can't update chapter on capture.`);
                return false;
            }
            ...
            if (!vod) {
                log(LOGLEVEL.FATAL, "automator.updateGame", `Tried to load VOD ${capture_id} for chapter update but errored.`);
                log(LOGLEVEL.INFO, "automator.updateGame", `Resetting online status on ${this.getLogin()}.`);
                KeyValue.getInstance().delete(`${this.getLogin()}.online`);
                return false;
            }

            if (!no_run_check && !await vod.getCapturingStatus(true)) {
                log(LOGLEVEL.ERROR, "automator.updateGame", `VOD ${vod.basename} is not capturing, skipping chapter update. Removing online status.`);
                KeyValue.getInstance().delete(`${this.getLogin()}.online`);
                return false;
            }
            ...
            if (from_cache) {
                if (this.channel) {
                    ...
                    log(LOGLEVEL.ERROR, "automator.updateGame", `No chapter data for ${this.getLogin()} found in cache.`);
                    return false;
                }
            } else if (this.payload_eventsub && "title" in this.payload_eventsub.event) {
                if (!this.payload_eventsub || !this.payload_eventsub.event) {
                    log(LOGLEVEL.ERROR, "automator.updateGame", `Tried to get event for ${this.getLogin()} but it was not available.`);
                    return false;
                }
                event = this.payload_eventsub.event as ChannelUpdateEvent;
                chapter_data = await this.getChapterData(event);
            } else {
                log(LOGLEVEL.ERROR, "automator.updateGame", `No last resort event for ${this.getLogin()} not available.`);
                return false;
            }

            if (!chapter_data) {
                log(LOGLEVEL.ERROR, "automator.updateGame", `No chapter data for ${this.getLogin()} found.`);
                return false;
            }

            log(LOGLEVEL.SUCCESS, "automator.updateGame", `Channel data for ${this.getLogin()} fetched from ${from_cache ? "cache" : "notification"}.`);
            ...
            if (!("title" in this.payload_eventsub.event)) {
                log(LOGLEVEL.ERROR, "automator.updateGame", `Event type was wrong for ${this.getLogin()}`);
                return false;
            }
            ...
            log(LOGLEVEL.INFO, "automator.updateGame", `Channel ${this.getLogin()} not capturing, saving channel data to cache: ${event.category_name} (${event.title})`);
            KeyValue.getInstance().setObject(`${this.getLogin()}.chapterdata`, chapter_data);
            if (this.channel) {
            ...
            */
            ...
            if (!this.channel?.no_capture && is_live && !this.channel?.is_capturing) {
                if (!this.getVodID()) {
                    log(LOGLEVEL.WARNING, "automator.updateGame", `Channel ${this.getLogin()} does not have a stream id, cannot start downloading from chapter update.`);
                } else {
                    log(LOGLEVEL.INFO, "automator.updateGame", `Channel ${this.getLogin()} status is online but not capturing, starting capture from chapter update.`);
                    this.download();
                }
            }
    ...
    public providerArgs(): string[] {
        ...
        if (Config.getInstance().cfg("capture.twitch-ttv-lol-plugin")) {
            ...
            if (Config.getInstance().hasValue("capture.twitch-proxy-playlist")) {
                ...
                log(LOGLEVEL.WARNING, "automator.providerArgs", "Twitch proxy playlist is enabled but no URL is set.");
            }
    ...
    public captureTicker(source: "stdout" | "stderr", raw_data: Buffer): void {
        ...
        if (proxy_match) {
            log(LOGLEVEL.INFO, "automator.captureVideo", `Using playlist proxy: ${proxy_match[1]}`);
        }
